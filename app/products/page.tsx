...
// Fallback static products shown when backend has no data yet
const STATIC_PRODUCTS: (Product & { category: string; unit: string; badge: string })[] = [
  ...
  { id: 4, name: "Ibuprofen 400mg", description: "Anti-inflammatory tablets · 24 tabs", price: 2.10, stock: 60, category: "Pain Relief", unit: "pack", badge: "OTC" },
...
function guessCategory(name: string, desc: string): string {
  ...
  if (text.includes("amoxicillin") || text.includes("antibiotic") || text.includes("cipro") || text.includes("metronidazole")) return "Antibiotics";
  if (text.includes("panado") || text.includes("ibuprofen") || text.includes("paracetamol") || text.includes("pain") || text.includes("aspirin")) return "Pain Relief";
  if (text.includes("lotion") || text.includes("cream") || text.includes("skin")) return "Skincare";
  if (text.includes("baby") || text.includes("infant") || text.includes("nappy")) return "Baby & Mother";
  return "Wellness";
}
    ...
    <>
      ...
      <div style={{ background: "var(--green-pale)", padding: "3rem 2rem", borderBottom: "2px solid var(--green-light)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: "2.5rem", fontWeight: 900, color: "var(--black)" }}>Our Products</h1>
          <p style={{ color: "var(--gray-dark)", marginTop: "0.5rem" }}>Quality medicines and health products — MCAZ approved, competitively priced.</p>
          {error && (
            <p style={{ marginTop: "0.5rem", fontSize: 13, color: "var(--red-cross)", background: "var(--red-light)", padding: "6px 12px", borderRadius: 6, display: "inline-block" }}>
              ⚠️ {error}
            </p>
          )}
        </div>
      ...
      {/* Layout */}
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "3rem 2rem", display: "grid", gridTemplateColumns: "240px 1fr", gap: "2.5rem" }}>

        {/* Filter Sidebar */}
        <aside style={{ background: "var(--white)", border: "1.5px solid var(--green-light)", borderRadius: "var(--radius-lg)", padding: "1.5rem", height: "fit-content" }}>
          ...
          {CATEGORIES.map((cat) => (
            <div key={cat} onClick={() => setActiveCategory(cat)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", cursor: "pointer" }}>
              <input
                type="radio"
                readOnly
              ...
              <label style={{ fontSize: 14, color: "var(--charcoal)", cursor: "pointer" }}>
                {cat}
                <span style={{ fontSize: 12, color: "var(--gray-mid)", marginLeft: 4 }}>
        ...
        <div>
          {loading ? (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1.5rem" }}>
              {[1,2,3,4,5,6].map(i => (
                <div key={i} style={{ height: 260, background: "var(--gray-light)", borderRadius: "var(--radius-lg)", animation: "pulse 1.5s infinite" }} />
              ))}
            </div>
          ) : filtered.length === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1.5rem" }}>
              {filtered.map((p) => (
                <ProductCard key={p.id} product={p} onAddToCart={() => showToast(`${p.name} added to cart — $${p.price.toFixed(2)}`)} />
              ))}
            </div>
          )}
        </div>
...
function ProductCard({
  ...
  return (
    <div
      ...
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLDivElement).style.borderColor = "var(--gray-light)";
    ...
    >
      {/* Image area */}
      <div style={{
        height: 130, background: "var(--green-pale)",
        display: "flex", alignItems: "center", justifyContent: "center",
        borderBottom: "1px solid var(--green-light)", position: "relative",
      }}>
        <PillIcon color={isRx ? "#ffcdd2" : "#c8e6c9"} accent={isRx ? "#c62828" : "#2e7d32"} />
        <span style={{
          position: "absolute", top: 10, right: 10,
          background: isRx ? "var(--red-cross)" : "var(--green-mid)",
        ...
        {product.stock === 0 && (
          <span style={{
            ...
            background: "var(--gray-mid)", color: "white",
      ...
      <div style={{ padding: "1rem" }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: "var(--green-mid)", marginBottom: 4 }}>
          {product.category}
        </div>
        <div style={{ fontFamily: "var(--font-display)", fontSize: "1rem", fontWeight: 700, color: "var(--black)", marginBottom: 4 }}>
          {product.name}
        </div>
        <div style={{ fontSize: 12, color: "var(--gray-mid)", marginBottom: 12 }}>{product.description}</div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ fontFamily: "var(--font-display)", fontSize: "1.2rem", fontWeight: 900, color: "var(--green-dark)" }}>
            ${product.price.toFixed(2)}
            <span style={{ fontSize: 12, fontWeight: 400, color: "var(--gray-mid)" }}> / {product.unit}</span>
          </div>
          <button
            onClick={(e) => { e.stopPropagation(); onAddToCart(); }}
            disabled={product.stock === 0}
            style={{
              background: product.stock === 0 ? "var(--gray-light)" : "var(--green-mid)",
              color: product.stock === 0 ? "var(--gray-mid)" : "white",
